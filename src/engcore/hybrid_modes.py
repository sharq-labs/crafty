HYBRID_MODE_CONFIGS = {
    # Starting ablation configurations, not claimed optimal.
    "fast": {
        "refit_interval": 6,
        "screen_pool": 50_000,
...
def get_hybrid_mode(name):
    key = str(name).lower()
    if key not in HYBRID_MODE_CONFIGS:
        raise KeyError(
            f"Unknown mode {name!r}. "
            f"Choose one of: {', '.join(HYBRID_MODE_CONFIGS)}"
        )
    return dict(HYBRID_MODE_CONFIGS[key])
