# Engineering AI Core V0.3.0.1 — Stacked Dual-GP Bayesian Optimization
...
## V0.3.0.1 runtime patch

V0.3.0 used the member identifier `matern25`, but initialized four diagnostic
counters with the shorter prefix `matern`. The Matern GP fit could therefore
finish successfully and then crash only while incrementing its diagnostic
counter.

V0.3.0.1 standardizes the counters on the real member identifier:

```text
matern25_optimized_fits
matern25_warm_only_fits
matern25_fit_failures
matern25_fit_rollbacks
```

The engine now also validates this schema during construction so the mismatch
fails immediately if a member is renamed in the future.
